...
//component('borrowReturn',{
 //   templateUrl:'borrowReturnBook.html',
    controller('borrowReturnBookCntrl', ['$routeParams','$scope','$window','customerData','customerBookData','$location','readJsonFile','bookId','$modalInstance',
    	...
    	$scope.showCustDetails = function(){
    		var custTellength = $scope.custTel;
    		if(custTellength.length == 10){
    			$scope.custDetail = customerData.getCustSpecificData($scope.custTel);
    			if($scope.custDetail.length == '0'){
    				$window.alert("Customer is not added in Database. Please add it first to proceed further.");
    			}
    			else{
